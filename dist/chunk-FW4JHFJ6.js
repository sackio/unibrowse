...
async function createServerWithTools(options) {
  ...
  async function handleBrowserMessage(ws, message) {
    ...
    console.log(`[Server] Handling message: ${type2} (id: ${id})`);
    try {
      ...
      const result = await tool.handle(globalContext, payload || {});
  ...
  wss.on("connection", (websocket) => {
    ...
    if (globalContext.hasWs()) {
      console.log("[Server] Closing existing WebSocket connection");
