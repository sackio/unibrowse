...
const UPDATED_CODE = `(params) => {
  ...
  try {
    ...
    // ENHANCED: Extract delivery information with full details
    ...
    if (result.product.price && deliveryCost) {
      try {
...
async function updateMacro() {
  ...
  try {
    ...
    const result = await macros.updateOne(
      { name: 'amazon_get_product_info' },
      {
        $set: {
          ...
          version: newVersion,
          updatedAt: new Date(),
          returnType: '{ success: boolean, product: { asin, title, price, unitPrice?, deliveryCost, deliveryInfo, totalCost, availability, rating, ratingCount, socialProof?, brand?, variations, requiresCustomization, description?, features?, specifications?, aboutItem? } }'
        }
      }
    );
    ...
    if (result.modifiedCount === 1) {
      ...
      console.log('\nChanges:');
      console.log('- Enhanced deliveryCost extraction with better fallbacks');
      console.log('- Added deliveryInfo (full delivery text block)');
      console.log('- Added totalCost calculation (price + delivery)');
      console.log('- Normalized FREE delivery to $0.00');
      console.log('- More robust delivery information extraction');
    } else {
